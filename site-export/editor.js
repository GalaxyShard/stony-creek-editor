    ...
    const nodes = Object.fromEntries(nodeExtensions.map(extension => {
        ...
        const schema = cleanUpSchemaItem({
            ...
            attrs: Object.fromEntries(extensionAttributes.map(extensionAttribute => {
                var _a;
                return [extensionAttribute.name, { default: (_a = extensionAttribute === null || extensionAttribute === undefined ? undefined : extensionAttribute.attribute) === null || _a === undefined ? undefined : _a.default }];
            })),
        });
...
function run$1$1(config) {
    ...
    $from.parent.type.spec.code
        // check for code mark
        || !!((_a = ($from.nodeBefore || $from.nodeAfter)) === null || _a === undefined ? undefined : _a.marks.find(mark => mark.type.spec.code))) {
        return false;
    }
    let matched = false;
...
class Mark {
    ...
    static handleExit({ editor, mark }) {
        ...
        if (isAtEnd) {
            ...
            const removeMark = currentMarks.find(m => (m === null || m === undefined ? undefined : m.type.name) === mark.name);
            if (removeMark) {
...
const createClipboardPasteEvent = (text) => {
    ...
    (_a = event.clipboardData) === null || _a === undefined ? undefined : _a.setData('text/html', text);
    return event;
};
/**
        ...
        return new Plugin({
            ...
            view(view) {
                const handleDragstart = (event) => {
                    var _a;
                    dragSourceElement = ((_a = view.dom.parentElement) === null || _a === undefined ? undefined : _a.contains(event.target))
...
function getTextBetween(startNode, range, options) {
    ...
    startNode.nodesBetween(from, to, (node, pos, parent, index) => {
        ...
        if (node.isText) {
            text += (_a = node === null || node === undefined ? undefined : node.text) === null || _a === undefined ? undefined : _a.slice(Math.max(from, pos) - pos, to - pos); // eslint-disable-line
        }
    });
    return text;
}
...
const blur = () => ({ editor, view }) => {
    requestAnimationFrame(() => {
        ...
        if (!editor.isDestroyed) {
            ...
            (_a = window === null || window === undefined ? undefined : window.getSelection()) === null || _a === undefined ? undefined : _a.removeAllRanges();
        }
    });
    return true;
};
...
const keyboardShortcut = name => ({ editor, view, tr, dispatch, }) => {
    ...
    capturedTransaction === null || capturedTransaction === undefined ? undefined : capturedTransaction.steps.forEach(step => {
...
const splitListItem = (typeOrName, overrideAttrs = {}) => ({ tr, state, dispatch, editor, }) => {
    ...
    if ($from.parent.content.size === 0 && $from.node(-1).childCount === $from.indexAfter(-1)) {
        ...
        if (dispatch) {
            ...
            const nextType = ((_a = type.contentMatch.defaultType) === null || _a === undefined ? undefined : _a.createAndFill(newNextTypeAttributes)) || undefined;
...
class Editor extends EventEmitter {
    ...
    dispatchTransaction(transaction) {
        ...
        if (this.isCapturingTransaction) {
            ...
            transaction.steps.forEach(step => { var _a; return (_a = this.capturedTransaction) === null || _a === undefined ? undefined : _a.step(step); });
            return;
        }
    ...
    return new Plugin({
        ...
        props: {
            handleClick: (view, pos, event) => {
                ...
                const href = (_a = link === null || link === undefined ? undefined : link.href) !== null && _a !== undefined ? _a : attrs.href;
                const target = (_b = link === null || link === undefined ? undefined : link.target) !== null && _b !== undefined ? _b : attrs.target;
                if (link && href) {
        ...
        return [
            markPasteRule({
                ...
                getAttributes: match => {
                    var _a;
                    return {
                        href: (_a = match.data) === null || _a === undefined ? undefined : _a.href,
                    };
                },
            }),
        ];
...
async function sendMessageToApp(_body) {
    // await fetch("/post", {
    //     method: "POST",
    //     body: body,
    // });
...
function createTextEditor(element) {
    ...
    async function onSave() {
        ...
        await sendMessageToApp(serializeCommand("replace-element") + serializedLocation + html + "\n");
    ...
    let editorInstance = new Editor({
        ...
        extensions: [
            Document, Paragraph, Text$1, BulletList, ListItem, HardBreak, Heading, Bold, Italic, Link, SmallMark
        ],
...
function createImageEditor(element) {
    ...
    async function onSave() {
        ...
        await sendMessageToApp(serializeCommand("replace-element") + serializedLocation + serializeElementHtml(element));
...
function createAddElementDropdown(element) {
    ...
    async function finalizeAddition(newElement) {
        await sendMessageToApp(serializeCommand("add-element") + serializePlacementLocation(element) + serializeElementHtml(newElement));
...
function createHoverToolbar(element) {
    ...
    removeButton.addEventListener("click", async _ => {
        ...
        await sendMessageToApp(serializeCommand("remove-element") + serializePlacementLocation(element));
    ...
    downButton.addEventListener("click", async _ => {
        ...
        if (nextElement) {
            ...
            await sendMessageToApp(serializeCommand("move-element") + serializePlacementLocation(element) + (placeInsideNext ? "in-start\n" : "after\n") + serializePlacementLocation(nextElement));
                ...
                && !element.parentElement.parentElement.classList.contains("row"))
            {
                ...
                await sendMessageToApp(serializeCommand("move-element") + serializePlacementLocation(element) + "after\n" + serializePlacementLocation(element.parentElement));
    ...
    upButton.addEventListener("click", async _ => {
        ...
        if (prevElement) {
            ...
            await sendMessageToApp(serializeCommand("move-element") + serializePlacementLocation(element) + (placeInsidePrev ? "in-end\n" : "before\n") + serializePlacementLocation(prevElement));
        ...
        {
            // allow moving images up to the body, and anything else up one level but not to the body or out of a column
            await sendMessageToApp(serializeCommand("move-element") + serializePlacementLocation(element) + "before\n" + serializePlacementLocation(element.parentElement));
